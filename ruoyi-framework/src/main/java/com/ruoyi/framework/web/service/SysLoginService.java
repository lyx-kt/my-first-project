...
import com.ruoyi.framework.security.EmailAuthenticationToken;
import com.ruoyi.framework.security.PhoneAuthenticationToken;
...
public class SysLoginService
{
     ...
     * @param code 验证码
     * @param uuid 唯一标识
     * @param loginType 登录类型 (username/phone/email)
     * @param phone 手机号
     * @param email 邮箱
     * @return token
     */
    public String login(String username, String password, String code, String uuid, 
                       String loginType, String phone, String email)
    {
        // 验证码校验
        validateCaptcha(username != null ? username : (phone != null ? phone : email), code, uuid);
        
        // 确定登录账号
        String loginAccount = username;
        if ("phone".equals(loginType) && StringUtils.isNotEmpty(phone)) {
            loginAccount = phone;
        } else if ("email".equals(loginType) && StringUtils.isNotEmpty(email)) {
            loginAccount = email;
        }
        ...
        try
        {
            ...
            if ("phone".equals(loginType)) {
                PhoneAuthenticationToken authenticationToken = new PhoneAuthenticationToken(loginAccount, password);
                AuthenticationContextHolder.setContext(authenticationToken);
                authentication = authenticationManager.authenticate(authenticationToken);
            } else if ("email".equals(loginType)) {
                EmailAuthenticationToken authenticationToken = new EmailAuthenticationToken(loginAccount, password);
                AuthenticationContextHolder.setContext(authenticationToken);
                authentication = authenticationManager.authenticate(authenticationToken);
            } else {
                // 默认用户名登录
